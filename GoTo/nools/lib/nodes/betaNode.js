...
Node.extend({
    ...
    instance: {
        ...
        constructor: function () {
            ...
            //for asserting rules at run time
            this.new = 0;
        },
        ...
        assertLeft: function (context,runTime,ruleBetaNodes) {
            ...
            if(this.__count == 35 || this.count == 45 )
                debugger;
            while (++i < l) {
                this.propagateFromLeft(context, rm[i].data,runTime,ruleBetaNodes);
            }
        },
        ...
        assertRight: function (context,runTime,ruleBetaNodes) {
            ...
            while (++i < l) {
                this.propagateFromRight(context, lm[i].data,runTime,ruleBetaNodes);
            }
        },
        ...
        propagateFromLeft: function (context, rc,runTime,ruleBetaNodes) {
            this.__propagate("assert", this.__addToMemoryMatches(rc, context, context.clone(null, null, context.match.merge(rc.match))),runTime,ruleBetaNodes);
        },

        propagateFromRight: function (context, lc,runTime,ruleBetaNodes) {
            this.__propagate("assert", this.__addToMemoryMatches(context, lc, lc.clone(null, null, lc.match.merge(context.match))),runTime,ruleBetaNodes);
        },
