...
BetaNode.extend({
    ...
    instance: {
        ...
        propagateFromLeft: function (context, rm,runTime,ruleBetaNodes) {
            var mr;
            if ((mr = this.constraint.match(context, rm)).isMatch) {
                this.__propagate("assert", this.__addToMemoryMatches(rm, context, context.clone(null, null, mr)),runTime,ruleBetaNodes);
            }
            return this;
        },

        propagateFromRight: function (context, lm,runTime,ruleBetaNodes) {
            var mr;
            if ((mr = this.constraint.match(lm, context)).isMatch) {
                this.__propagate("assert", this.__addToMemoryMatches(context, lm, context.clone(null, null, mr)),runTime,ruleBetaNodes);
            }
            return this;
        },
