...
declare({
    instance: {
        ...
        assertRuleAtRunTime: function (rule) {
            /*if(isLooping)
                nextTick(assertRuleAtRunTime)
            else*/

            var terminalNode = new TerminalNode(this.bucket, this.__ruleCount++, rule, this.agendaTree);
            var ruleTypeNodes = [], ruleBetaNodes = [], that = this;
            //                                                   side undefined 
            this.__addToNetwork(rule, rule.pattern, terminalNode,undefined,ruleTypeNodes,ruleBetaNodes);
            this.__mergeJoinNodes();
            this.terminalNodes.push(terminalNode);
            this.print();
            ruleTypeNodes.forEach(function(typeNode,index){
                var type = typeNode.type;
                //get all the facts of this type for the typeNode
                that.workingMemory.getFactsByTypeString(type.toLowerCase(),function(facts){
                    for(var j = 0; j < facts.length ; j++){
                        typeNode.assert(facts[j],true,ruleBetaNodes);
                    }
                })
            })
        ...
        __createTypeNode: function (rule, pattern) {
            var ret = new TypeNode(pattern.get("constraints")[0],pattern.type);
        ...
        __createJoinNode: function (rule, pattern, outNode, side,ruleBetaNodes) {
            var joinNode;
            ...
            //keep track of all the beta nodes of the rule
            if(ruleBetaNodes)
                ruleBetaNodes.push(joinNode);
        ...
        __addToNetwork: function (rule, pattern, outNode, side,ruleTypeNodes,ruleBetaNodes) {
            if (pattern instanceof ObjectPattern) {
                if (!(pattern instanceof InitialFactPattern) && (!side || side === "left")) {
                    this.__createBetaNode(rule, new CompositePattern(new InitialFactPattern(), pattern), outNode, side,ruleTypeNodes,ruleBetaNodes);
                } else {
                    this.__createAlphaNode(rule, pattern, outNode, side,ruleTypeNodes,ruleBetaNodes);
                }
            } else if (pattern instanceof CompositePattern) {
                this.__createBetaNode(rule, pattern, outNode, side,ruleTypeNodes,ruleBetaNodes);
            }
        },

        __createBetaNode: function (rule, pattern, outNode, side,ruleTypeNodes,ruleBetaNodes) {
            var joinNode = this.__createJoinNode(rule, pattern, outNode, side,ruleBetaNodes);
            this.__addToNetwork(rule, pattern.rightPattern, joinNode, "right",ruleTypeNodes,ruleBetaNodes);
            this.__addToNetwork(rule, pattern.leftPattern, joinNode, "left",ruleTypeNodes,ruleBetaNodes);
            outNode.addParentNode(joinNode);
            return joinNode;
        },


        __createAlphaNode: function (rule, pattern, outNode, side,ruleTypeNodes,ruleBetaNodes) {
