...
declare({
    ...
    instance: {
        ...
        constructor: function (flowContainer) {
            this.recency = 0;
        ...
        getFactsByTypeString: function (type,cb) {
            var collection = db.collection(type.toLowerCase());
            var that = this;
            collection.find(function(err, doc){
                for (var i = doc.length - 1; i >= 0; i--) {
                    var instance;
                    if(doc[i].object.__type.toLowerCase() == "initialfact")
                        instance = new InitialFact();
                    else{
                        var Template = that.flowContainer.getDefined(doc[i].object.__type.toLowerCase());
                        instance = new Template(doc[i].object);
                        instance.objectId = doc[i].objectId;
                    }
                    doc[i].object = instance
                };
                cb(doc);
            });
